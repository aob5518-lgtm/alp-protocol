...
createServer(async (req, res) => {
  try {
    ...
    if (req.method === "POST" && url.pathname === "/v1/auth/siwe") { const body = JSON.parse(await requestBody(req)) as {message:string; signature:`0x${string}`}; return json(res, 200, {session:issueSession(await verifySiwe(pool, body.message, body.signature, url.host))}); }
