...
contract NodeDividendDistributor is AccessControl, ReentrancyGuard {
    ...
    event NodeDividendClaimed(
        uint256 indexed epochId, uint256 indexed nodeId, address indexed owner, uint256 amount
    );

    constructor(
        IERC20 rewardToken_,
        address nodeDividendTreasury_,
        NodeRegistry nodeRegistry_,
        address admin
    ) {
        if (
            address(rewardToken_) == address(0) || nodeDividendTreasury_ == address(0)
                || address(nodeRegistry_) == address(0) || admin == address(0)
        ) {
            revert ZeroAddress();
        }
    ...
    function claim(uint256 epochId, uint256 nodeId, uint256 amount, bytes32[] calldata proof)
        external
        nonReentrant
    {
        ...
        bytes32 leaf =
            keccak256(bytes.concat(keccak256(abi.encode(epochId, nodeId, msg.sender, amount))));
