...
contract BuybackExecutor is AccessControl, ReentrancyGuard {
    ...
    constructor(
        ...
        if (
            address(paymentToken_) == address(0) || buybackTreasury_ == address(0)
                || address(oracle_) == address(0) || buybackRecipient_ == address(0)
    ...
    function configureToken(address token, bool allowed, uint256 maximumTradeAmount)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function _queueTrade(address router, address[] memory path, uint128 amountIn)
        private
        returns (bytes32 tradeId)
    {
    ...
    function executeTrade(bytes32 tradeId, uint256 minimumOut, uint256 deadline)
        external
        nonReentrant
        returns (uint256 amountOut)
    {
        ...
        if (block.timestamp < trade.executableAt) {
            revert TradeNotReady(tradeId, trade.executableAt, block.timestamp);
        }
        ...
        if (minimumOut < liveOracleMinimumOut) {
            revert InsufficientMinimumOut(minimumOut, liveOracleMinimumOut);
        }
        ...
        uint256[] memory amounts = IPancakeV2Router(trade.router)
            .swapExactTokensForTokens(trade.amountIn, minimumOut, path, buybackRecipient, deadline);
        amountOut = amounts[amounts.length - 1];
        if (amountOut < minimumOut) revert RouterOutputTooLow(amountOut, minimumOut);
        inputToken.forceApprove(trade.router, 0);
        emit BuybackExecuted(
            tradeId, trade.router, trade.tokenIn, trade.tokenOut, trade.amountIn, amountOut
        );
    }

    function oracleMinimumOut(address tokenIn, address tokenOut, uint256 amountIn)
        public
        view
        returns (uint256)
    {
    ...
    function oracleMinimumOutPath(address[] calldata path, uint256 amountIn)
        external
        view
        returns (uint256)
    {
    ...
    function _oracleMinimumOutPath(address[] memory path, uint256 amountIn)
        private
        view
        returns (uint256)
    {
        ...
        for (uint256 i; i < path.length - 1; ++i) {
            ...
            if (tokenIn == tokenOut || !oracle.isPriceValid(tokenIn)) {
                revert PriceUnavailable(tokenIn);
            }
    ...
    function _validateTrade(address router, address[] memory path, uint256 amountIn) private view {
        ...
        if (path.length < 2 || path[0] != address(paymentToken) || amountIn == 0) {
            revert InvalidPath();
        }
