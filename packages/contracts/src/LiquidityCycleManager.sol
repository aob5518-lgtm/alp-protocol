...
contract LiquidityCycleManager is ILiquidityCycleManager {
    ...
    event SellCredited(
        address indexed account, uint8 indexed cycle, uint256 grossAmount, uint256 cycleSoldAmount
    );
    ...
    function settleOverdueCycle(address account, uint8 cycleIndex)
        external
        returns (uint256 burnedNow)
    {
        ...
        uint256 missing =
            required > cycle.soldAmount[cycleIndex] ? required - cycle.soldAmount[cycleIndex] : 0;
        cycle.burnDebt += missing;
        burnedNow = _settleDebt(account, cycle);
        cycle.forcedBurned[cycleIndex] = burnedNow;
        emit ForcedBurnSettled(
            account, cycleIndex, required, cycle.soldAmount[cycleIndex], burnedNow, cycle.burnDebt
        );
    }
        ...
        returns (uint64 startTime, uint256 baseline, uint256 required)
    {
        ...
        AccountCycle storage cycle = _cycles[account];
        return (
            cycle.cycleStartTime[cycleIndex],
            cycle.cycleBaseline[cycleIndex],
            cycle.requiredAmount[cycleIndex]
        );
    }
    ...
    function _outstandingObligation(AccountCycle storage cycle, uint8 cycleIndex)
        private
        view
        returns (uint256)
    {
    ...
    function _totalOutstandingObligation(AccountCycle storage cycle)
        private
        view
    ...
    function _settleDebt(address account, AccountCycle storage cycle)
        private
        returns (uint256 burnedNow)
    {
